...
**Versão atual:** **v0.14** (deployada 2026-05-26 — regra de multi-objeção: roteiro cobre TODAS as objeções marcadas, não só a "mais pesada").
...
| **v0.14 (ATIVA)** | **2026-05-26** | **DEPLOYADA** | **Regra de multi-objeção.** Bug v0.13.1 observado no lead 29354429 (#6 "Vou ver com a direção" + #11 "Sem prioridade agora"): roteiro inteiro focou em #6, #11 apareceu só no `proximo_passo`. Patch: nova seção "REGRA DE MULTI-OBJEÇÃO" após as 5 falas, instruindo: quando 2+ objeções marcadas (excluindo N/A), roteiro DEVE cobrir TODAS — combinando em 1 fala (pergunta com 3 eixos) OU dedicando falas separadas, e fala 5 (PRÓXIMO PASSO) DEVE amarrar todas. Exemplo correto pra #6+#11 incluído inline. Validação no mesmo lead 29354429 (exec 259289): fala 2 cobriu ambas ("Me conta: como normalmente acontece esse alinhamento com a direção? E hoje o que pesa mais — agenda do ano, orçamento, ou outras prioridades?"), fala 5 amarrou ambas ("alinhar material pra direção e entender prioridade do ano"). Watch: LLM copiou a fala 2 do exemplo praticamente literal — pode virar bug de repetição se múltiplos leads com #6+#11 vierem. Refino v0.14.1 deferido até observar reincidência. |
